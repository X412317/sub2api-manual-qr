...
// SubmitManualProof uploads a private proof image and transaction number.
func (h *PaymentHandler) SubmitManualProof(c *gin.Context) {
	subject, ok := requireAuth(c)
	if !ok {
		return
	}
	...
	c.Request.Body = http.MaxBytesReader(c.Writer, c.Request.Body, service.ManualProofMaxUploadBytes+(1<<20))
	transactionNo := c.PostForm("transaction_no")
	header, err := c.FormFile("proof")
	if err != nil {
		response.BadRequest(c, "Payment proof image is required")
		return
	}
	if header.Size > service.ManualProofMaxUploadBytes {
		response.BadRequest(c, "Payment proof image exceeds 5 MB")
		return
	}
	file, err := header.Open()
	if err != nil {
		response.BadRequest(c, "Failed to read payment proof image")
		return
	}
	defer file.Close()
	proof, err := manual.SubmitProof(c.Request.Context(), orderID, subject.UserID, transactionNo, file)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
func (h *PaymentHandler) GetManualProof(c *gin.Context) {
	...
	proof, err := manual.GetUserProof(c.Request.Context(), orderID, subject.UserID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
func (h *PaymentHandler) GetManualQR(c *gin.Context) {
	subject, ok := requireAuth(c)
	if !ok {
		return
	}
	...
	image, err := manual.GetOrderQRImage(c.Request.Context(), orderID, subject.UserID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
