...
func (h *PaymentHandler) SetStepUpDeps(totpService *service.TotpService, userService *service.UserService) {
...
func (h *PaymentHandler) ListOrders(c *gin.Context) {
	...
	orders, total, err := h.paymentService.AdminListOrders(c.Request.Context(), userID, service.OrderListParams{
		...
		ManualReviewStatus: c.Query("review_status"),
	})
	if err != nil {
		response.ErrorFrom(c, err)
		return
...
// GetManualReviewSummary returns the number of proofs awaiting review.
func (h *PaymentHandler) GetManualReviewSummary(c *gin.Context) {
	manual := h.paymentService.ManualPayment()
	if manual == nil {
		response.Success(c, gin.H{"pending": 0})
		return
	}
	count, err := manual.PendingReviewSummary(c.Request.Context())
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
	response.Success(c, gin.H{"pending": count})
}

func (h *PaymentHandler) GetManualProof(c *gin.Context) {
	...
	manual := h.paymentService.ManualPayment()
	if manual == nil {
		response.Error(c, http.StatusServiceUnavailable, "Manual payment service is unavailable")
		return
	}
	if c.Query("image") == "1" {
		image, err := manual.GetAdminProofImage(c.Request.Context(), orderID)
		if err != nil {
			response.ErrorFrom(c, err)
			return
		}
		writeManualPrivateImage(c, image)
		return
	}
	proof, err := manual.GetAdminProof(c.Request.Context(), orderID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
	response.Success(c, proof)
}
...
func (h *PaymentHandler) ReviewManualProof(c *gin.Context) {
	orderID, ok := parseIDParam(c, "id")
	if !ok {
		return
	}
	...
	if strings.EqualFold(strings.TrimSpace(req.Action), "approve") &&
		!middleware2.EnforceStepUpAlways(c, h.totpService, h.userService) {
		return
	}
	...
	proof, err := manual.ReviewProof(c.Request.Context(), orderID, service.ManualProofReviewRequest{
		Action: req.Action, ReceivedAmount: req.ReceivedAmount, Reason: req.Reason, ReviewerUserID: subject.UserID,
	})
	if err != nil {
		response.ErrorFrom(c, err)
		return
...
func (h *PaymentHandler) ListManualQRAssets(c *gin.Context) {
	...
	assets, err := manual.ListQRAssets(c.Request.Context(), providerID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
func (h *PaymentHandler) SaveManualQRAsset(c *gin.Context) {
	providerID, ok := parseIDParam(c, "id")
	if !ok {
		return
	}
	c.Request.Body = http.MaxBytesReader(c.Writer, c.Request.Body, service.ManualQRMaxUploadBytes+(512<<10))
	channel := c.PostForm("channel")
	var amount *float64
	if raw := strings.TrimSpace(c.PostForm("amount")); raw != "" {
	...
	header, err := c.FormFile("qr")
	if err != nil {
		response.BadRequest(c, "QR image is required")
		return
	}
	if header.Size > service.ManualQRMaxUploadBytes {
		response.BadRequest(c, "QR image exceeds 1 MB")
		return
	}
	file, err := header.Open()
	if err != nil {
		response.BadRequest(c, "Failed to read QR image")
		return
	}
	defer file.Close()
	manual := h.paymentService.ManualPayment()
	...
	asset, err := manual.SaveQRAsset(c.Request.Context(), providerID, channel, amount, file)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
func (h *PaymentHandler) DeleteManualQRAsset(c *gin.Context) {
	providerID, ok := parseIDParam(c, "id")
	if !ok {
		return
	}
	...
	if err := manual.DeleteQRAsset(c.Request.Context(), providerID, assetID); err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
func (h *PaymentHandler) GetManualQRAssetImage(c *gin.Context) {
	providerID, ok := parseIDParam(c, "id")
	if !ok {
		return
	}
	...
	image, err := manual.GetQRAssetImage(c.Request.Context(), providerID, assetID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
func writeManualPrivateImage(c *gin.Context, image *service.ManualPrivateImage) {
	if image == nil {
