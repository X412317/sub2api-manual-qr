...
func TestEasyPayQueryOrderStatusMapping(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:        "failure code cannot be overridden by a paid trade status",
			body:        `{"code":0,"trade_status":"TRADE_SUCCESS","money":"12.34","trade_no":"conflicting-123"}`,
			wantStatus:  payment.ProviderStatusPending,
			wantTradeNo: "conflicting-123",
			wantAmount:  12.34,
		},
		{
...
func TestEasyPayQueryOrderFallsBackToGETForFixedQRGateway(t *testing.T) {
	t.Parallel()
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"code":1,"data":{"status":"1","amount":10,"transaction_id":"gateway-fixed-1"}}`))
	}))
	defer server.Close()

	provider := newTestEasyPay(t, server.URL)
	resp, err := provider.QueryOrder(context.Background(), "fixed-qr-order")
	if err != nil {
		t.Fatalf("QueryOrder returned error: %v", err)
	}
	if resp.Status != payment.ProviderStatusPaid || resp.Amount != 10 || resp.TradeNo != "gateway-fixed-1" {
...
func TestEasyPayQueryOrderRejectsNonSuccessHTTPResponses(t *testing.T) {
	t.Parallel()

	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusBadGateway)
		// A success-looking body must not override the transport error.
		_, _ = w.Write([]byte(`{"code":1,"trade_status":"TRADE_SUCCESS","money":"10"}`))
	}))
	defer server.Close()

	provider := newTestEasyPay(t, server.URL)
	_, err := provider.QueryOrder(context.Background(), "http-error-order")
	if err == nil {
		t.Fatal("QueryOrder returned nil error for non-success HTTP responses")
	}
