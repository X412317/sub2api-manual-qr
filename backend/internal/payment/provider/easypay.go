...
func (e *EasyPay) QueryOrder(ctx context.Context, tradeNo string) (*payment.QueryOrderResponse, error) {
	...
	// Most EasyPay installations accept a form POST. Some fixed-QR gateways
	// expose the same read-only endpoint as GET only, so retry a malformed or
	// non-JSON POST response with the documented query-string form.
	body, postStatus, postErr := e.postRaw(ctx, endpoint, params)
	var postParseErr error
	if postErr == nil {
		if postStatus < http.StatusOK || postStatus >= http.StatusMultipleChoices {
			postErr = fmt.Errorf("HTTP %d: %s", postStatus, summarizeEasyPayResponse(body))
		} else if response, parseErr := parseEasyPayQueryResponse(body, tradeNo, e.MerchantIdentityMetadata()); parseErr == nil {
			return response, nil
		} else {
			postParseErr = parseErr
		}
	}

	body, getErr := e.get(ctx, endpoint, params)
	var getParseErr error
	if getErr == nil {
		if response, parseErr := parseEasyPayQueryResponse(body, tradeNo, e.MerchantIdentityMetadata()); parseErr == nil {
			return response, nil
		} else {
			getParseErr = parseErr
		}
	}
	if postErr != nil {
		return nil, fmt.Errorf("easypay query: %w", postErr)
	}
	if getErr != nil {
		return nil, fmt.Errorf("easypay query: %w", getErr)
	}
	if getParseErr != nil {
		return nil, fmt.Errorf("easypay parse query: %w", getParseErr)
	}
	if postParseErr != nil {
		return nil, fmt.Errorf("easypay parse query: %w", postParseErr)
	}
	return nil, fmt.Errorf("easypay query returned an invalid response")
}

// parseEasyPayQueryResponse accepts the response shapes used by the common
// EasyPay implementations: fields may be top-level or nested under data, and
// status/money may be encoded as strings or JSON numbers.
func parseEasyPayQueryResponse(body []byte, fallbackTradeNo string, metadata map[string]string) (*payment.QueryOrderResponse, error) {
	...
	tradeStatus, hasTradeStatus := easyPayQueryField(payload, "trade_status")
	statusValue, hasStatus := easyPayQueryField(payload, "status")
	codeValue, hasCode := easyPayQueryField(payload, "code")
	responseCodeOK := !hasCode || easyPayNumericValue(codeValue) == easypayCodeSuccess
	status := payment.ProviderStatusPending
	if responseCodeOK && hasTradeStatus {
		// An explicitly present but empty/non-success trade_status is treated as
		// pending; do not let a contradictory numeric status mark it paid.
		if easyPayStatusIsPaid(tradeStatus) {
			status = payment.ProviderStatusPaid
		}
	} else if responseCodeOK && hasStatus {
		if easyPayStatusIsPaid(statusValue) {
			status = payment.ProviderStatusPaid
		} else if n, ok := easyPayNumericValueOK(statusValue); ok && n == easypayStatusPaid {
			status = payment.ProviderStatusPaid
		}
	}

	amountValue, _ := easyPayQueryField(payload, "money")
	if amountValue == nil {
		for _, key := range []string{"amount", "total_amount", "pay_amount"} {
			if value, ok := easyPayQueryField(payload, key); ok {
				amountValue = value
				break
			}
		}
	}
	amount, _ := easyPayNumericValueOK(amountValue)

	responseTradeNo := fallbackTradeNo
	for _, key := range []string{"trade_no", "transaction_id", "trans_no"} {
		if value, ok := easyPayQueryField(payload, key); ok {
			if candidate := strings.TrimSpace(easyPayStringValue(value)); candidate != "" {
				responseTradeNo = candidate
				break
			}
		}
	}
...
func easyPayQueryField(payload map[string]any, key string) (any, bool) {
	if value, ok := payload[key]; ok {
		return value, true
	}
	if nested, ok := payload["data"].(map[string]any); ok {
		if value, found := nested[key]; found {
			return value, true
		}
	}
	return nil, false
...
func easyPayStringValue(value any) string {
...
func easyPayNumericValueOK(value any) (float64, bool) {
	switch typed := value.(type) {
	case json.Number:
...
func easyPayNumericValue(value any) float64 {
	parsed, _ := easyPayNumericValueOK(value)
	return parsed
}

func easyPayStatusIsPaid(value any) bool {
	normalized := strings.ToUpper(strings.TrimSpace(easyPayStringValue(value)))
	switch normalized {
	case tradeStatusSuccess, "SUCCESS", "PAID", "PAY_SUCCESS", "TRADE_FINISHED":
		return true
	default:
		return false
	}
...
func (e *EasyPay) VerifyNotification(_ context.Context, rawBody string, _ map[string]string) (*payment.PaymentNotification, error) {
	...
	tradeStatus, hasTradeStatus := params["trade_status"]
	if hasTradeStatus && easyPayStatusIsPaid(tradeStatus) {
		status = payment.ProviderStatusSuccess
	} else if !hasTradeStatus {
		if easyPayStatusIsPaid(params["status"]) {
			status = payment.ProviderStatusSuccess
		} else if numericStatus, parseErr := strconv.Atoi(strings.TrimSpace(params["status"])); parseErr == nil && numericStatus == easypayStatusPaid {
			status = payment.ProviderStatusSuccess
		}
	}
	amount, _ := strconv.ParseFloat(strings.TrimSpace(firstNonEmptyEasyPayParam(params, "money", "amount", "total_amount", "pay_amount")), 64)
	...
	return &payment.PaymentNotification{
		TradeNo: firstNonEmptyEasyPayParam(params, "trade_no", "transaction_id", "trans_no"),
		OrderID: firstNonEmptyEasyPayParam(params, "out_trade_no", "order_id"),
		Amount:  amount, Status: status, RawData: rawBody, Metadata: metadata,
	}, nil
}

func firstNonEmptyEasyPayParam(params map[string]string, keys ...string) string {
