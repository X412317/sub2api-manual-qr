...
func TestUpdateProviderInstanceCanDisableManualQRWithPendingOrders(t *testing.T) {
	t.Parallel()
	...
	disabled := false
	updated, err := svc.UpdateProviderInstance(ctx, instance.ID, UpdateProviderInstanceRequest{Enabled: &disabled})
	require.NoError(t, err)
