...
const leaderLockDBFallbackTimeout = 2 * time.Second
...
// tryAcquireSingletonLeaderLockWithError is the diagnostic form used by jobs
// that must surface coordination failures. A Redis timeout can exhaust the
// caller's acquisition context, so the Postgres fallback gets its own bounded
// context instead of inheriting an already-cancelled one.
func tryAcquireSingletonLeaderLockWithError(ctx context.Context, cache LeaderLockCache, db *sql.DB, key, owner string, ttl time.Duration) (func(), bool, error) {
	...
	if db != nil {
		...
		cancel := func() {}
		if cacheErr != nil {
			dbCtx, cancel = context.WithTimeout(context.Background(), leaderLockDBFallbackTimeout)
		}
		defer cancel()
		release, acquired, err := tryAcquireDBAdvisoryLockWithError(dbCtx, db, hashAdvisoryLockID(key))
		if err != nil {
			if cacheErr != nil {
				return nil, false, fmt.Errorf("leader lock cache failed (%v), database fallback failed: %w", cacheErr, err)
			}
			return nil, false, fmt.Errorf("database leader lock failed: %w", err)
		}
