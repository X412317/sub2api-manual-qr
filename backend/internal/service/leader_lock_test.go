...
func TestTryAcquireSingletonLeaderLock_CacheTimeoutGetsFreshDBFallbackContext(t *testing.T) {
	...
	cache := &fakeLeaderLockCache{acquireErr: errors.New("redis timeout")}
	exhaustedCtx, cancel := context.WithCancel(context.Background())
	cancel()

	release, acquired, lockErr := tryAcquireSingletonLeaderLockWithError(exhaustedCtx, cache, db, "payment:test", "instance", time.Minute)
