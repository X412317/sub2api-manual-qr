...
func (p *paymentOrderLifecycleQueryProvider) SupportedTypes() []payment.PaymentType {
	if p.ProviderKey() == payment.TypeEasyPay {
		// EasyPay is an aggregator; its instance can serve the visible methods.
...
func TestReconcilePendingPaymentOrdersBackfillsOverdueEasyPayOrder(t *testing.T) {
	ctx := context.Background()
	client := newPaymentOrderLifecycleTestClient(t)
	...
	provider := &paymentOrderLifecycleQueryProvider{
		...
		resp: &payment.QueryOrderResponse{
			TradeNo: "easypay-upstream-trade-7",
			Status:  payment.ProviderStatusPaid,
			Amount:  7,
		},
	}
	...
	require.NoError(t, err)
	require.Equal(t, OrderStatusCompleted, reloaded.Status)
	require.Equal(t, 7.0, userRepo.getByIDUser.Balance)
}

func TestReconcilePendingPaymentOrdersIgnoresNilProviderResponse(t *testing.T) {
	ctx := context.Background()
	client := newPaymentOrderLifecycleTestClient(t)
	user, err := client.User.Create().
		SetEmail("nil-query@example.com").SetPasswordHash("hash").SetUsername("nil-query-user").Save(ctx)
	require.NoError(t, err)
	_, err = client.PaymentOrder.Create().
		SetUserID(user.ID).SetUserEmail(user.Email).SetUserName(user.Username).
		SetAmount(3).SetPayAmount(3).SetFeeRate(0).SetRechargeCode("NIL-QUERY").
		SetOutTradeNo("sub2_nil_query").SetPaymentType(payment.TypeAlipay).
		SetPaymentTradeNo("").SetOrderType(payment.OrderTypeBalance).
		SetStatus(OrderStatusPending).SetExpiresAt(time.Now().Add(time.Hour)).
		SetClientIP("127.0.0.1").SetSrcHost("api.example.com").Save(ctx)
	require.NoError(t, err)
	_, err = client.PaymentOrder.Create().
		SetUserID(user.ID).SetUserEmail(user.Email).SetUserName(user.Username).
		SetAmount(3).SetPayAmount(3).SetFeeRate(0).SetRechargeCode("MANUAL-SKIP").
		SetOutTradeNo("sub2_manual_skip").SetPaymentType(payment.TypeWxpay).
		SetPaymentTradeNo("").SetProviderKey(payment.TypeManualQR).
	...
	registry := payment.NewRegistry()
	provider := &paymentOrderLifecycleQueryProvider{key: payment.TypeAlipay, resp: nil}
	registry.Register(provider)
