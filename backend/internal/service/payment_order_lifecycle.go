...
const (
	...
	paymentOrderExpiryBatchLimit = 100
)
...
func (s *PaymentService) cancelCore(ctx context.Context, o *dbent.PaymentOrder, fs, op, ad string) (string, error) {
	if paymentOrderIsManualQR(o) && s.manualPayment != nil {
		blockAnyProof := strings.HasPrefix(op, "user:")
		changed, blocked, err := s.manualPayment.TryTransitionPendingOrder(ctx, o.ID, fs, blockAnyProof)
		if err != nil {
			return "", err
		}
		if blocked {
			...
			return "", infraerrors.Conflict("MANUAL_PAYMENT_PROOF_SUBMITTED", "order with submitted payment proof cannot be cancelled")
		}
		if changed {
...
// ReconcilePendingPaymentOrders actively checks pending automatic
// payment orders so a missed provider notification does not leave a paid
// recharge unfulfilled. Overdue PENDING rows are included because they indicate
// a previous expiry sweep did not finish. Manual QR orders are excluded because
// they require administrator proof review and have no upstream order to query.
func (s *PaymentService) ReconcilePendingPaymentOrders(ctx context.Context) (int, error) {
	orders, err := s.entClient.PaymentOrder.Query().
		Where(
			...
			paymentorder.Or(
				...
				paymentorder.ProviderKeyNEQ(payment.TypeManualQR),
			),
		).
		Order(dbent.Asc(paymentorder.FieldExpiresAt), dbent.Asc(paymentorder.FieldCreatedAt)).
	...
	for _, order := range orders {
		// Older manual orders may have only a provider snapshot and a NULL
		// provider_key, so keep the exclusion defensive at iteration time too.
		if paymentOrderIsManualQR(order) {
			continue
		}
...
func paymentOrderIsManualQR(order *dbent.PaymentOrder) bool {
	if order == nil {
		return false
	}
	if strings.EqualFold(strings.TrimSpace(psStringValue(order.ProviderKey)), payment.TypeManualQR) {
		return true
	}
	if snapshot := psOrderProviderSnapshot(order); snapshot != nil {
		return strings.EqualFold(strings.TrimSpace(snapshot.ProviderKey), payment.TypeManualQR)
	}
	return false
}

// ReconcilePendingWxpayOrders is retained for callers built against the
// previous API. The reconciliation is now provider-agnostic and includes
// EasyPay, official Alipay/WeChat Pay, Stripe, and Airwallex orders.
...
func (s *PaymentService) ExpireTimedOutOrders(ctx context.Context) (int, error) {
	...
	orders, err := s.entClient.PaymentOrder.Query().
		...
		Order(dbent.Asc(paymentorder.FieldExpiresAt), dbent.Asc(paymentorder.FieldCreatedAt)).
		Limit(paymentOrderExpiryBatchLimit).
		All(ctx)
	if err != nil {
	...
	for _, o := range orders {
		...
		outcome, expireErr := s.cancelCore(ctx, o, OrderStatusExpired, "system", "order expired")
		if expireErr != nil {
			slog.Warn("failed to expire payment order", "orderID", o.ID, "error", expireErr)
			if firstErr == nil {
				firstErr = fmt.Errorf("expire order %d: %w", o.ID, expireErr)
			}
			continue
		}
