...
func TestGetBasePaymentType(t *testing.T) {
	...
	tests := []struct {
		...
		{payment.TypeManualQR, payment.TypeManualQR},
...
func TestBuildVisibleMethodSourceAvailability(t *testing.T) {
	...
	instances := []*dbent.PaymentProviderInstance{
		...
		{ProviderKey: payment.TypeManualQR, SupportedTypes: "alipay,wxpay"},
	}
	...
	if !got[VisibleMethodSourceManualAlipay] {
		t.Fatalf("expected %q to be available", VisibleMethodSourceManualAlipay)
	}
	if !got[VisibleMethodSourceManualWechat] {
		t.Fatalf("expected %q to be available", VisibleMethodSourceManualWechat)
	}
}
...
func TestManualProviderEconomicsAreEnforcedAtConfigurationTime(t *testing.T) {
	ctx := context.Background()
	client := newPaymentConfigServiceTestClient(t)
	repo := &paymentConfigSettingRepoStub{values: map[string]string{
		SettingBalanceRechargeMult: "1.00",
		SettingRechargeFeeRate:     "0.00",
	}}
	svc := &PaymentConfigService{entClient: client, settingRepo: repo}

	_, err := client.PaymentProviderInstance.Create().
		SetProviderKey(payment.TypeManualQR).
		SetName("Manual QR").
		SetConfig("{}").
		...
		Save(ctx)
	if err != nil {
		t.Fatalf("create manual provider fixture: %v", err)
	}

	fee := 0.01
	err = svc.UpdatePaymentConfig(ctx, UpdatePaymentConfigRequest{RechargeFeeRate: &fee})
	if infraerrors.Reason(err) != "MANUAL_PAYMENT_FEE_MUST_BE_ZERO" {
		t.Fatalf("fee update error = %v, want MANUAL_PAYMENT_FEE_MUST_BE_ZERO", err)
	}

	multiplier := 0.5
	err = svc.UpdatePaymentConfig(ctx, UpdatePaymentConfigRequest{BalanceRechargeMultiplier: &multiplier})
	if infraerrors.Reason(err) != "MANUAL_PAYMENT_MULTIPLIER_MUST_BE_ONE" {
		t.Fatalf("multiplier update error = %v, want MANUAL_PAYMENT_MULTIPLIER_MUST_BE_ONE", err)
	}

	fee = 0
	multiplier = 1
	helpText := "manual payment enabled"
	err = svc.UpdatePaymentConfig(ctx, UpdatePaymentConfigRequest{
		RechargeFeeRate: &fee, BalanceRechargeMultiplier: &multiplier, HelpText: &helpText,
	})
	if err != nil {
		t.Fatalf("valid manual payment economics rejected: %v", err)
	}
	if repo.values[SettingHelpText] != helpText {
		t.Fatalf("help text = %q, want %q", repo.values[SettingHelpText], helpText)
	}
}

func TestEnabledManualProviderCannotBeCreatedWithInvalidEconomics(t *testing.T) {
	ctx := context.Background()
	client := newPaymentConfigServiceTestClient(t)
	svc := &PaymentConfigService{
		entClient: client,
		settingRepo: &paymentConfigSettingRepoStub{values: map[string]string{
			SettingBalanceRechargeMult: "0.50",
			SettingRechargeFeeRate:     "0.00",
		}},
	}

	_, err := svc.CreateProviderInstance(ctx, CreateProviderInstanceRequest{
		ProviderKey: payment.TypeManualQR, Name: "Manual QR", SupportedTypes: []string{payment.TypeAlipay}, Enabled: true,
	})
	if infraerrors.Reason(err) != "MANUAL_PAYMENT_MULTIPLIER_MUST_BE_ONE" {
		t.Fatalf("enabled provider error = %v, want MANUAL_PAYMENT_MULTIPLIER_MUST_BE_ONE", err)
	}

	provider, err := svc.CreateProviderInstance(ctx, CreateProviderInstanceRequest{
		ProviderKey: payment.TypeManualQR, Name: "Manual QR draft",
		SupportedTypes: []string{payment.TypeAlipayDirect, payment.TypeAlipay, payment.TypeWxpayDirect}, Enabled: false,
		PaymentMode: "redirect", RefundEnabled: true, AllowUserRefund: true,
	})
	if err != nil {
		t.Fatalf("disabled manual provider draft should be allowed: %v", err)
	}
	if provider.PaymentMode != "qrcode" || provider.RefundEnabled || provider.AllowUserRefund {
		t.Fatalf("manual provider invariants not forced: %+v", provider)
	}
	if provider.SupportedTypes != "alipay,wxpay" {
		t.Fatalf("manual provider types = %q, want canonical alipay,wxpay", provider.SupportedTypes)
	}
}
