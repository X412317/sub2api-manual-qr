...
type MethodLimits struct {
	...
	ManualQR     bool    `json:"manual_qr,omitempty"`
...
func (s *PaymentConfigService) UpdatePaymentConfig(ctx context.Context, req UpdatePaymentConfigRequest) error {
	...
	manualEnabled, err := s.hasEnabledManualQRProvider(ctx)
	if err != nil {
		return err
	}
	if manualEnabled {
		...
		feeRate := current.RechargeFeeRate
		multiplier := current.BalanceRechargeMultiplier
		if req.RechargeFeeRate != nil {
			feeRate = *req.RechargeFeeRate
		}
		if req.BalanceRechargeMultiplier != nil {
			multiplier = *req.BalanceRechargeMultiplier
		}
		if err := validateManualPaymentEconomics(feeRate, multiplier); err != nil {
			return err
		}
	}
