...
func (s *PaymentConfigService) GetAvailableMethodLimits(ctx context.Context) (*MethodLimitsResponse, error) {
	...
	for pt, insts := range typeInstances {
		...
		pcDecorateManualMethodLimits(&ml, insts)
...
func (s *PaymentConfigService) GetMethodLimits(ctx context.Context, types []string) ([]MethodLimits, error) {
	...
	for _, pt := range types {
		...
		ml.Currency = currency
		pcDecorateManualMethodLimits(&ml, matching)
		result = append(result, ml)
	}
	return result, nil
}

func pcDecorateManualMethodLimits(limits *MethodLimits, instances []*dbent.PaymentProviderInstance) {
	if limits == nil {
		return
	}
	for _, inst := range instances {
		if inst != nil && inst.ProviderKey == payment.TypeManualQR {
			limits.ManualQR = true
			limits.FixedAmounts = ManualPaymentFixedAmounts()
			limits.SingleMin = float64(manualPaymentFixedAmounts[0])
			limits.SingleMax = float64(manualPaymentFixedAmounts[len(manualPaymentFixedAmounts)-1])
			return
		}
	}
}
