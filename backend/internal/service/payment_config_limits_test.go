...
func TestPcDecorateManualMethodLimitsUsesFixedTiers(t *testing.T) {
	t.Parallel()

	limits := MethodLimits{PaymentType: payment.TypeAlipay, SingleMin: 1, SingleMax: 9999}
	manual := makeInstance(1, payment.TypeManualQR, payment.TypeAlipay, "")
	pcDecorateManualMethodLimits(&limits, []*dbent.PaymentProviderInstance{manual})

	require.True(t, limits.ManualQR)
	require.Equal(t, []int64{10, 20, 50, 100, 200, 500, 1000}, limits.FixedAmounts)
	require.Equal(t, 10.0, limits.SingleMin)
	require.Equal(t, 1000.0, limits.SingleMax)
}
