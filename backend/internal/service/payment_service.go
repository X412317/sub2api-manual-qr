...
type PaymentService struct {
	...
	manualPayment            *ManualPaymentService
...
func (s *PaymentService) SetManualPaymentService(manualPayment *ManualPaymentService) {
	s.manualPayment = manualPayment
}

func (s *PaymentService) ManualPayment() *ManualPaymentService {
	if s == nil {
		return nil
	}
...
func (s *PaymentService) CleanupManualPaymentProofs(ctx context.Context, now time.Time) (int, error) {
	if s == nil || s.manualPayment == nil {
		return 0, nil
	}
	return s.manualPayment.CleanupTerminalProofFiles(ctx, now)
}
