...
func (s *PaymentConfigService) hasEnabledManualQRProvider(ctx context.Context) (bool, error) {
	if s == nil || s.entClient == nil {
		return false, nil
	}
	exists, err := s.entClient.PaymentProviderInstance.Query().
		Where(
			paymentproviderinstance.ProviderKeyEQ(payment.TypeManualQR),
			paymentproviderinstance.EnabledEQ(true),
		).
		Exist(ctx)
	if err != nil {
		return false, fmt.Errorf("check enabled manual QR provider: %w", err)
	}
	return exists, nil
}

func (s *PaymentConfigService) validateManualProviderEconomics(ctx context.Context) error {
	cfg, err := s.GetPaymentConfig(ctx)
	if err != nil {
		return err
	}
	return validateManualPaymentEconomics(cfg.RechargeFeeRate, cfg.BalanceRechargeMultiplier)
}
...
var validProviderKeys = map[string]bool{
	...
	payment.TypeManualQR: true,
}

func (s *PaymentConfigService) CreateProviderInstance(ctx context.Context, req CreateProviderInstanceRequest) (*dbent.PaymentProviderInstance, error) {
	if req.ProviderKey == payment.TypeManualQR {
		normalizedTypes, err := normalizeManualProviderTypes(req.SupportedTypes)
		if err != nil {
			return nil, err
		}
		req.SupportedTypes = normalizedTypes
		if req.Enabled {
			if err := s.validateManualProviderEconomics(ctx); err != nil {
				return nil, err
			}
		}
		req.Config = map[string]string{}
		req.PaymentMode = "qrcode"
...
func normalizeManualProviderTypes(types []string) ([]string, error) {
	if len(types) == 0 {
		return nil, infraerrors.BadRequest("VALIDATION_ERROR", "manual QR provider must enable alipay, wxpay, or both")
	}
	...
	for _, paymentType := range types {
		...
		if paymentType != payment.TypeAlipay && paymentType != payment.TypeWxpay {
			return nil, infraerrors.BadRequest("VALIDATION_ERROR", "manual QR provider only supports alipay and wxpay")
		}
		if !seen[paymentType] {
			seen[paymentType] = true
			normalized = append(normalized, paymentType)
		}
	}
	if len(seen) == 0 {
		return nil, infraerrors.BadRequest("VALIDATION_ERROR", "manual QR provider must enable at least one channel")
	}
...
func (s *PaymentConfigService) UpdateProviderInstance(ctx context.Context, id int64, req UpdateProviderInstanceRequest) (*dbent.PaymentProviderInstance, error) {
	...
	if current.ProviderKey == payment.TypeManualQR {
	...
	// Manual QR orders carry an immutable asset snapshot and remain reviewable
	// after their provider is disabled. Allow operators to stop new manual orders
	// immediately during incident response or rollback.
	if req.Enabled != nil && !*req.Enabled && current.ProviderKey != payment.TypeManualQR {
		count, err := getPendingOrderCount()
		if err != nil {
			return nil, err
		}
