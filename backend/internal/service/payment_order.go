...
func (s *PaymentService) CreateOrder(ctx context.Context, req CreateOrderRequest) (*CreateOrderResponse, error) {
	...
	if err := s.validateManualCreateOrder(req, cfg, sel); err != nil {
		return nil, err
	}
...
func (s *PaymentService) createOrderInTx(ctx context.Context, req CreateOrderRequest, user *User, plan *dbent.SubscriptionPlan, cfg *PaymentConfig, orderAmount, limitAmount, feeRate, payAmount float64, sel *payment.InstanceSelection) (*dbent.PaymentOrder, error) {
	...
	if sel != nil && sel.ProviderKey == payment.TypeManualQR {
		manualSnapshot, err := s.manualPayment.ResolveOrderAsset(ctx, sel.InstanceID, req.PaymentType, payAmount)
		if err != nil {
			return nil, err
		}
		if providerSnapshot == nil {
			providerSnapshot = map[string]any{
				"schema_version":       2,
				"provider_instance_id": strings.TrimSpace(sel.InstanceID),
				"provider_key":         payment.TypeManualQR,
				"payment_mode":         "qrcode",
				"currency":             payment.DefaultPaymentCurrency,
			}
		}
		providerSnapshot["currency"] = payment.DefaultPaymentCurrency
		providerSnapshot["manual_qr"] = manualSnapshot
	}
...
func (s *PaymentService) invokeProvider(ctx context.Context, order *dbent.PaymentOrder, req CreateOrderRequest, cfg *PaymentConfig, limitAmount float64, payAmountStr string, payAmount float64, plan *dbent.SubscriptionPlan, sel *payment.InstanceSelection) (*CreateOrderResponse, error) {
	...
	if sel.ProviderKey == payment.TypeManualQR {
		pr.QRCode = fmt.Sprintf("/api/v1/payment/orders/%d/manual-qr", order.ID)
	}
...
func (s *PaymentService) validateManualCreateOrder(req CreateOrderRequest, cfg *PaymentConfig, sel *payment.InstanceSelection) error {
	if sel == nil || sel.ProviderKey != payment.TypeManualQR {
		return nil
	}
	if req.OrderType != payment.OrderTypeBalance {
		return infraerrors.BadRequest("MANUAL_PAYMENT_BALANCE_ONLY", "manual QR payment only supports balance recharge")
	}
	if req.PaymentType != payment.TypeAlipay && req.PaymentType != payment.TypeWxpay {
		return infraerrors.BadRequest("MANUAL_PAYMENT_INVALID_CHANNEL", "manual QR payment channel must be alipay or wxpay")
	}
	if !isManualPaymentFixedAmount(req.Amount) {
		return infraerrors.BadRequest("MANUAL_PAYMENT_INVALID_TIER", "amount is not an allowed manual payment tier")
	}
	if cfg == nil {
		return infraerrors.ServiceUnavailable("MANUAL_PAYMENT_PROVIDER_MISCONFIGURED", "manual QR payment configuration is unavailable")
	}
	if err := validateManualPaymentEconomics(cfg.RechargeFeeRate, cfg.BalanceRechargeMultiplier); err != nil {
		return err
	}
	if strings.TrimSpace(sel.PaymentMode) != "qrcode" {
		return infraerrors.ServiceUnavailable("MANUAL_PAYMENT_PROVIDER_MISCONFIGURED", "manual QR provider must use qrcode payment mode")
	}
	...
	return s.manualPayment.available()
}
...
func (s *PaymentService) AdminListOrders(ctx context.Context, userID int64, p OrderListParams) ([]*dbent.PaymentOrder, int, error) {
	...
	if p.ManualReviewStatus != "" {
		if p.ManualReviewStatus != ManualProofStatusSubmitted || s.manualPayment == nil {
			return nil, 0, infraerrors.BadRequest("MANUAL_PAYMENT_INVALID_REVIEW_STATUS", "manual review status must be submitted")
		}
		ids, err := s.manualPayment.PendingReviewOrderIDs(ctx)
		if err != nil {
			return nil, 0, err
		}
		if len(ids) == 0 {
			return []*dbent.PaymentOrder{}, 0, nil
		}
		q = q.Where(paymentorder.IDIn(ids...))
	}
	if userID > 0 {
