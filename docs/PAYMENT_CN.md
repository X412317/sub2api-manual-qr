...
| **人工二维码** | 支付宝、微信支付 | 固定档位、用户提交付款凭证、管理员 TOTP 审核入账；仅支持余额充值 |

人工二维码的部署、配置、验收和回滚步骤见 [人工二维码分档充值运维指南](MANUAL_QR_PAYMENT_CN.md)。

> 支付宝官方 / 微信官方、易支付与人工二维码可以同时作为后台服务商实例存在，但前台始终只展示 `支付宝`、`微信支付` 两个可见按钮。管理员需要分别为这两个按钮选择唯一支付来源：官方、易支付或人工二维码。人工二维码不接收自动回调，只能用于余额充值并由管理员审核入账。
...
当前版本对用户统一展示支付方式，不区分官方渠道、易支付还是人工二维码：

- **支付宝**：后台启用后，需要额外指定该按钮路由到 `支付宝官方`、`易支付支付宝` 或 `人工二维码支付宝`
- **微信支付**：后台启用后，需要额外指定该按钮路由到 `微信官方`、`易支付微信` 或 `人工二维码微信`
...
使用管理员自行上传的支付宝、微信个人收款码，不需要第三方商户账号，也不会监听收款通知或伪造支付回调。该服务商只支持余额充值、固定档位和人工审核，退款始终关闭。

| 参数 | 说明 | 必填 |
|------|------|------|
| **支付渠道** | 支付宝、微信支付，可同时启用 | 是 |
| **通用收款码** | 对应渠道缺少定额码时使用 | 每个启用渠道至少一张 |
| **定额收款码** | 对应 `10、20、50、100、200、500、1000 CNY` 档位；下单时优先使用 | 否 |

二维码配置在服务商实例保存后进行。必须保持余额充值倍率为 `1`、手续费率为 `0%`；批准凭证前还必须为管理员配置 TOTP 二次验证。完整上线步骤见 [人工二维码分档充值运维指南](MANUAL_QR_PAYMENT_CN.md)。
...
自动支付服务商依赖回调，必须正确配置。人工二维码没有 Webhook，由管理员核对真实流水并审核凭证：
...
- 确保服务器防火墙允许支付平台的回调请求
- 系统会自动进行签名验证，防止伪造回调
- 自动支付服务商在支付成功后自动完成余额充值
- 人工二维码必须由管理员完成 TOTP 二次验证并批准凭证后才会入账
...
用户选择充值金额和支付方式
       ...
       ▼
  创建订单 (PENDING)
  ├─ 自动支付：校验限额 → 选择服务商 → 获取支付信息
  └─ 人工二维码：校验固定档位 → 固化定额码或通用码快照
       │
       ├───────────────────────────────┐
       ▼                               ▼
  自动支付渠道                     人工二维码
  ├─ EasyPay：扫码 / H5            ├─ 支付宝：尝试唤起 App，失败显示二维码
  ├─ 支付宝官方：扫码 / WAP         ├─ 微信：显示并可保存二维码
  ├─ 微信官方：Native / H5 / JSAPI  └─ 用户付款后提交截图和交易号
  └─ Stripe：Payment Element               │
       │                                    ▼
       ▼                              待人工审核（停止过期）
  回调验签 → PAID                    ├─ 驳回：填写原因并重置完整倒计时
       │                              └─ 批准：TOTP + 实收金额精确匹配 → PAID
       └───────────────────────────────┬────┘
                                       ▼
                         幂等充值到用户余额 → COMPLETED
```
