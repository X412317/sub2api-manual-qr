export default {
  ...
  apiDocs: {
    ...
    subtitle: '使用一套 API 密钥接入 OpenAI、Anthropic 与 Gemini 兼容协议。页面中的地址和示例会自动匹配当前站点配置。',
    baseUrl: 'API 地址',
    onThisPage: '本页目录',
    actions: {
      manageKeys: '管理 API 密钥',
      getKey: '获取 API 密钥',
      viewModels: '查看可用模型与价格',
      copy: '复制地址',
      copyCode: '复制代码',
      copied: '已复制'
    },
    ...
    quickStart: {
      title: '快速开始',
      description: '从创建密钥到发出请求只需要三步',
      step1Title: '创建 API 密钥',
      step1Description: '登录控制台，在 API 密钥页面创建密钥并选择可用分组。',
      step2Title: '选择模型',
      step2Description: '在模型广场确认模型 ID、计费方式和当前生效倍率。',
      step3Title: '发送请求',
      step3Description: '按客户端使用的协议选择兼容端点，并替换示例中的密钥与模型。',
      securityNote: 'API 密钥只会在创建时完整显示。请保存在服务端环境变量中，不要写入前端代码、公开仓库或聊天记录。'
    },
    authentication: {
      title: '身份验证',
      description: '所有模型请求都需要 API 密钥。推荐按客户端原生方式发送请求头，不再使用 URL 查询参数传递密钥。',
      bearer: 'OpenAI 兼容',
    ...
    firstRequest: {
      title: '发送首个请求',
      description: '选择你的客户端协议。将 your-model-id 替换为模型广场中实际可用的模型 ID。',
      protocol: '请求协议',
      responses: 'Responses',
    ...
    endpoints: {
      title: '端点一览',
      description: '常用兼容端点如下。端点最终是否可用取决于 API 密钥所属分组和模型能力。',
      method: '方法',
      path: '路径',
      purpose: '用途',
      models: '读取当前密钥可见的模型列表',
      responses: 'OpenAI Responses API，适合新项目和工具调用',
      chat: 'OpenAI Chat Completions 兼容请求',
      messages: 'Anthropic Messages 兼容请求',
      countTokens: '请求前估算消息 Token 数量',
      gemini: 'Gemini generateContent 原生请求',
      images: '同步图片生成，需模型与分组支持',
      embeddings: '向量嵌入，仅支持具备对应能力的平台'
    },
    sdk: {
      title: 'SDK 配置',
      description: '官方 SDK 通常只需替换 API 密钥和 Base URL。建议通过环境变量注入密钥。'
    },
    errors: {
      title: '错误处理',
      description: '客户端应记录响应状态码与 request id，并只对可重试错误使用指数退避。',
      badRequest: '请求参数、模型名称或消息格式不正确。',
      unauthorized: 'API 密钥缺失、格式错误或已经失效。',
      forbidden: '当前用户、密钥或分组无权访问该模型。',
      rateLimited: '触发并发、RPM、配额或上游限流，请读取错误信息后退避重试。',
      server: '网关或上游服务暂时异常。保留 request id，并在短暂退避后有限次数重试。'
    },
    faq: {
      title: '常见问题',
      modelQuestion: '模型名称应该填什么？',
      modelAnswer: '使用模型广场或 GET /v1/models 返回的完整模型 ID。不同分组可用模型可能不同，请以 API 密钥所属分组为准。',
      balanceQuestion: '价格和余额按什么单位计算？',
      balanceAnswer: '模型广场的 Token 价格按美元每百万 Token 展示，实际扣费会应用分组倍率、高峰倍率及管理员配置的定价规则。',
      streamQuestion: '是否支持流式输出？',
      streamAnswer: '支持。对于 Responses、Chat Completions 或 Messages 请求，将 stream 设为 true，并让客户端持续读取事件流。',
      errorQuestion: '遇到 429 或 5xx 应该怎样重试？',
      errorAnswer: '使用带随机抖动的指数退避，并设置最大重试次数。不要立即并发重放大量请求；持续失败时请携带 request id 联系站点支持。'
    }
  },
  ...
  modelPlaza: {
    title: '模型广场',
    eyebrow: '实时模型目录',
    description: '查找可用模型，对比实付价格与官方参考价',
    loading: '加载中...',
    empty: '暂无可展示的分组',
    loadFailed: '加载模型广场失败',
    loadFailedHint: '模型数据暂时不可用，请稍后重试。',
    retry: '重新加载',
    noSearchResult: '没有匹配的模型',
    anonymousHint: '登录后可查看你的专属分组与专属倍率',
    signInForRates: '登录查看专属价格',
    viewDocs: '查看 API 文档',
    resetFilters: '清除筛选',
    resultSummary: '找到 {groups} 个分组，共 {models} 个模型',
    pricingNote: '实付价格根据当前生效倍率计算，仅供下单前参考；最终费用以实际请求用量和账单记录为准。',
    controls: {
      priceMode: '价格显示模式', paid: '实付', official: '官方', unit: '价格单位',
      perMillion: '/1M', perThousand: '/1K', sort: '排序', latest: '最新', name: '名称', price: '价格',
      view: '视图模式', grid: '卡片视图', list: '列表视图', details: '详情',
      modelPricing: '按当前分组倍率计算的 Token 价格', tieredPricing: '包含分段定价', imagePricing: '按图片或请求计费'
    },
    ...
    table: {
      ...
      paidPrice: '实付价格(折后)',
      officialPrice: '官方价格',
      officialReference: '展开官方参考价',
      rate: '折扣倍率',
      unitPerMillion: '$ / 1M token',
      unitPerThousand: '$ / 1K token',
      perUnitRequest: '/ 次',
      perUnitImage: '/ 张',
      perRequest: '按次计费',
      perImage: '按图片计费'
