export default {
    settings: {
      ...
      payment: {
        ...
        manualQRAssetsTitle: '收款二维码',
        manualQRAssetsHint: '分别配置支付宝和微信的通用收款码与定额收款码。下单优先使用对应档位，未配置时回退通用码。',
        manualQRSaveFirst: '该服务商默认保持禁用。请先保存实例，再重新编辑并上传收款二维码，确认配置无误后启用。',
        manualQRGeneric: '通用码',
        manualQRFixed: '¥{amount} 定额码',
        manualQRNotUploaded: '未上传',
        manualQRUploadRules: '仅支持 PNG、JPEG、WebP，文件不超过 1 MB；图片必须包含与所选渠道匹配且可识别的收款二维码。',
        manualQRUploadSuccess: '收款二维码已保存',
        manualQRDeleteSuccess: '收款二维码已删除',
      },
      balanceNotify: {
        title: '余额不足提醒',
