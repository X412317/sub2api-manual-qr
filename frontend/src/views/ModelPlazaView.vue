<template>
  ...
  <AppLayout v-if="isEmbedded">
    <ModelPlazaContent :response="data" :loading="loading" :error="loadFailed" embedded @retry="load" />
  </AppLayout>
  ...
  <div v-else class="min-h-screen bg-gray-50 dark:bg-dark-950">
    ...
    <main class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
      <ModelPlazaContent :response="data" :loading="loading" :error="loadFailed" @retry="load" />
    </main>
  </div>
</template>
