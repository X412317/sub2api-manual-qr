...
import { getProviderVisibleMethods as getConfiguredProviderVisibleMethods } from "@/components/payment/providerConfig";
import {
...
const providerKeyOptions = computed(() => [
  ...
  { value: "manual_qr", label: t("admin.settings.payment.providerManualQR") },
]);

const enabledProviderKeyOptions = computed(() => {
  const enabled = form.payment_enabled_types;
  return providerKeyOptions.value.filter((opt) => {
    const methods = getConfiguredProviderVisibleMethods(opt.value);
    return methods.some((method) => enabled.includes(method));
  });
});
...
function getProviderVisibleMethods(
  ...
  return getConfiguredProviderVisibleMethods(provider.provider_key, provider.supported_types)
    .filter((method): method is "alipay" | "wxpay" => method === "alipay" || method === "wxpay");
}
