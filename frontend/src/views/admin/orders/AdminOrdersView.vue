      ...
      <div class="card p-4">
        <div class="flex flex-wrap items-center gap-3">
          ...
          <button
            type="button"
            class="btn"
            :class="orderFilters.review_status === 'submitted' ? 'btn-primary' : 'btn-secondary'"
            @click="toggleManualReviewFilter"
          >
            <Icon name="inbox" size="sm" />
            {{ t('payment.admin.manualReview.pendingQueue') }}
            <span
              ...
              :class="orderFilters.review_status === 'submitted' ? 'bg-white/20 text-white' : 'bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-300'"
            >
              {{ manualReviewCount > 99 ? '99+' : manualReviewCount }}
            </span>
          </button>
      ...
      <OrderTable :orders="orders" :loading="ordersLoading" show-user>
        <template #actions="{ row }">
          <div class="flex items-center gap-1">
            ...
            <button v-if="row.status === 'PENDING' && row.provider_key !== 'manual_qr'" @click="handleCancelOrder(row)" class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-yellow-600 hover:bg-yellow-50 dark:text-yellow-400 dark:hover:bg-yellow-900/20">
              <Icon name="x" size="sm" />
              {{ t('payment.orders.cancel') }}
            </button>
            <button v-if="row.provider_key === 'manual_qr'" @click="showOrderDetail(row)" class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-amber-700 hover:bg-amber-50 dark:text-amber-300 dark:hover:bg-amber-900/20">
              <Icon name="clipboard" size="sm" />
              {{ t('payment.admin.manualReview.review') }}
            </button>
            ...
            <button v-else-if="row.provider_key !== 'manual_qr' && (row.status === 'COMPLETED' || row.status === 'PARTIALLY_REFUNDED')" @click="openRefundDialog(row)" class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-red-600 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-900/20">
    ...
    <BaseDialog :show="showDetailDialog" :title="t('payment.admin.orderDetail')" width="wide" @close="showDetailDialog = false">
      <div v-if="selectedOrder" class="space-y-4">
        ...
        <ManualPaymentReviewPanel
          v-if="selectedOrder.provider_key === 'manual_qr'"
          :order="selectedOrder"
          @reviewed="handleManualReviewed"
        />
        <!-- Audit Logs -->
...
const orderFilters = reactive({ status: '', payment_type: '', order_type: '', review_status: '' })
...
const manualReviewCount = ref(0)
let reviewSummaryTimer: ReturnType<typeof setInterval> | null = null
...
async function loadOrders() {
  ...
  try {
    const res = await adminPaymentAPI.getOrders({
      ...
      review_status: orderFilters.review_status || undefined,
    })
...
function toggleManualReviewFilter() {
  orderFilters.review_status = orderFilters.review_status === 'submitted' ? '' : 'submitted'
  orderPagination.page = 1
  loadOrders()
}

async function loadManualReviewSummary() {
  try {
    const response = await adminPaymentAPI.getManualReviewSummary()
    manualReviewCount.value = response.data.pending || 0
...
async function handleManualReviewed() {
  await Promise.all([loadOrders(), loadManualReviewSummary()])
  if (selectedOrder.value) await showOrderDetail(selectedOrder.value)
}
...
onMounted(() => {
  loadOrders()
  loadManualReviewSummary()
  reviewSummaryTimer = setInterval(loadManualReviewSummary, 30000)
})

onBeforeUnmount(() => {
  if (reviewSummaryTimer) clearInterval(reviewSummaryTimer)
})
</script>
