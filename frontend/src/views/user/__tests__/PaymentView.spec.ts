...
describe('PaymentView manual QR constraints', () => {
  beforeEach(() => {
    vi.useRealTimers()
    routeState.path = '/purchase'
    routeState.query = {}
  ...
  it('uses only the fixed recharge tiers exposed by a manual provider', async () => {
    ...
    getCheckoutInfo.mockReset().mockResolvedValue(checkoutInfoFixture({
      methods: {
        alipay: {
          ...base,
          manual_qr: true,
          fixed_amounts: [10, 20, 50, 100, 200, 500, 1000],
          single_min: 10,
          single_max: 1000,
        },
      },
    ...
    expect(amountInput.props('fixedOnly')).toBe(true)
    expect(amountInput.props('amounts')).toEqual([10, 20, 50, 100, 200, 500, 1000])
  })

  it('shows a manual method as unavailable for subscription checkout', async () => {
    const wrapper = await mountSubscriptionConfirm({
      method: {
        manual_qr: true,
        fixed_amounts: [10, 20, 50, 100, 200, 500, 1000],
