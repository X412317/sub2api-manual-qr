    ...
    <div class="mx-auto max-w-4xl space-y-6">
      ...
      <template v-else>
        ...
        <template v-if="paymentPhase === 'paying'">
          <ManualPaymentPanel
            v-if="paymentState.providerKey === 'manual_qr'"
        ...
        <template v-else>
          ...
          <template v-if="activeTab === 'recharge'">
            ...
            <div class="card p-6">
              <AmountInput
                v-model="amount"
                :amounts="selectedAmountOptions"
                :min="globalMinAmount"
                :max="globalMaxAmount"
                :fixed-only="selectedIsManualQR"
...
const selectedIsManualQR = computed(() => selectedLimit.value?.manual_qr === true)
const selectedAmountOptions = computed(() => selectedIsManualQR.value
  ? (selectedLimit.value?.fixed_amounts?.length ? selectedLimit.value.fixed_amounts : [10, 20, 50, 100, 200, 500, 1000])
...
const feeRate = computed(() => selectedIsManualQR.value ? 0 : (checkout.value?.recharge_fee_rate ?? 0))
...
const canSubmit = computed(() =>
  validAmount.value > 0
    && (!selectedIsManualQR.value || selectedAmountOptions.value.includes(validAmount.value))
...
const subMethodOptions = computed<PaymentMethodOption[]>(() => {
  ...
  return enabledMethods.value.map((type) => {
    ...
    return {
      ...
      display_name: ml?.display_name,
      fee_rate: ml?.fee_rate ?? 0,
      available: ml?.manual_qr !== true
        && ml?.available !== false
        && amountFitsMethod(subscriptionTotalAmountForCurrency(price, currency), type),
    }
  })
})
...
watch(selectedIsManualQR, (isManual) => {
  if (!isManual || selectedAmountOptions.value.includes(validAmount.value)) return
  amount.value = null
})
