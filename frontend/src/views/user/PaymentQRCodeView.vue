...
import { isRecoverablePaymentStatus, shouldActivelyRecoverPayment } from '@/components/payment/paymentRecovery'
...
let pollInFlightUsesForcedVerify = false
...
async function tryRecoverPendingOrder(order: PaymentOrder, forceVerify = false): Promise<PaymentOrder> {
  if (!shouldActivelyRecoverPayment({
    orderProviderKey: order.provider_key,
    checkoutProviderKey: providerKey.value,
    paymentType: paymentType.value,
  })) return order
  const outTradeNo = String(order.out_trade_no || '').trim()
  if (!outTradeNo || !isRecoverablePaymentStatus(order.status)) return order
  const now = Date.now()
...
async function pollStatus(forceVerify = false): Promise<void> {
  if (!orderId.value || completed.value || expired.value) return
  // 防重入：接口响应慢于 3 秒轮询间隔时避免并发重叠请求与重复跳转。
  if (pollInFlight) {
    const activePoll = pollInFlight
    const activePollWasForced = pollInFlightUsesForcedVerify
    await activePoll
    if (forceVerify && !activePollWasForced && !completed.value && !expired.value) {
      await pollStatus(true)
    }
    return
  }

  pollInFlightUsesForcedVerify = forceVerify
  const activePoll = (async () => {
    const order = await paymentStore.pollOrderStatus(orderId.value)
    if (!order || completed.value || expired.value) return
    // 定时器已被 cleanup 清除时不再处理迟到的普通响应；到期核验使用 forceVerify 明确允许继续。
    if (!pollTimer && !forceVerify) return
    const recoveredOrder = await tryRecoverPendingOrder(order, forceVerify)
    if (completed.value || expired.value) return
    if (recoveredOrder.status === 'COMPLETED' || recoveredOrder.status === 'PAID' || recoveredOrder.status === 'RECHARGING') {
      completed.value = true
      cleanup()
      await router.push({ path: '/payment/result', query: { order_id: String(orderId.value), status: 'success' } })
    } else if (recoveredOrder.status === 'EXPIRED' || recoveredOrder.status === 'CANCELLED' || recoveredOrder.status === 'FAILED') {
      cleanup()
      expired.value = true
    }
  })()
  ...
  try {
    ...
    if (pollInFlight === activePoll) {
      pollInFlight = null
      pollInFlightUsesForcedVerify = false
    }
  }
}

async function settleExpiredAfterVerification() {
  if (completed.value || expired.value) return
  ...
  try {
    ...
    // The page still reaches its local expiry state if the final lookup is unavailable.
  }
  if (!completed.value && !expired.value) {
    cleanup()
    expired.value = true
  }
}
...
function startCountdown(seconds: number) {
  ...
  if (remainingSeconds.value <= 0) {
    void settleExpiredAfterVerification()
    return
  }
...
onMounted(() => {
  ...
  providerKey.value = String(route.query.provider_key || '')
