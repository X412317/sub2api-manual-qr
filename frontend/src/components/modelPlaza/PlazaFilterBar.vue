<template>
  <section class="overflow-hidden rounded-lg border border-gray-200 bg-white dark:border-dark-800 dark:bg-dark-900/35" :aria-label="t('modelPlaza.filters.title')">
    <div class="border-b border-gray-200 p-3 dark:border-dark-800 sm:p-4">
      <label class="relative block w-full sm:max-w-md">
        ...
        <input
          ...
          class="input h-10 rounded-md py-2 pl-9 pr-9"
          @input="$emit('update:search', ($event.target as HTMLInputElement).value)"
        />
        <button v-if="search" type="button" class="absolute right-2 top-1/2 flex h-7 w-7 -translate-y-1/2 items-center justify-center rounded-md text-gray-400 transition-colors hover:bg-gray-100 hover:text-gray-700 dark:text-dark-500 dark:hover:bg-dark-700 dark:hover:text-white" :title="t('modelPlaza.filters.clearSearch')" @click="$emit('update:search', '')">
          <Icon name="x" size="xs" />
        </button>
      </label>
    </div>

    <div class="divide-y divide-gray-100 px-3 dark:divide-dark-800 sm:px-4">
      <div class="flex items-start gap-3 py-3">
        <span class="w-12 shrink-0 pt-1.5 text-xs font-semibold text-gray-400 dark:text-dark-500">
        ...
        <button
          ...
          :key="`platform-${p}`"
          type="button"
          class="inline-flex min-h-8 items-center gap-1.5 rounded-md px-2.5 py-1 text-sm font-medium transition disabled:cursor-not-allowed disabled:opacity-40 disabled:grayscale"
      ...
      <div class="flex items-start gap-3 py-3">
        ...
        <button
          type="button"
          class="min-h-8 rounded-md px-2.5 py-1 text-sm font-medium transition"
        ...
        <button
          ...
          :key="`group-${g.id}`"
          type="button"
          class="min-h-8 rounded-md px-2.5 py-1 text-sm font-medium transition disabled:cursor-not-allowed disabled:opacity-40 disabled:grayscale"
      ...
      <div class="flex items-start gap-3 py-3">
        ...
        <button
          ...
          :key="`rate-${r}`"
          type="button"
          class="min-h-8 rounded-md px-2.5 py-1 font-mono text-sm font-medium transition disabled:cursor-not-allowed disabled:opacity-40 disabled:grayscale"
