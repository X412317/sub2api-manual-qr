...
async function tryRecoverPendingOrder(order: PaymentOrder, forceVerify = false): Promise<PaymentOrder> {
  if (!shouldActivelyRecoverPayment({
    ...
    checkoutProviderKey: props.providerKey,
    paymentType: props.paymentType,
    mobileAlipayDeepLink: isMobileAlipayDeepLink.value,
  })) return order
  const outTradeNo = String(order.out_trade_no || '').trim()
  if (!outTradeNo) return order
  if (!isRecoverablePaymentStatus(order.status)) return order
  const now = Date.now()
...
async function pollStatus(forceVerify = false): Promise<void> {
  ...
  if (pollInFlight) {
    ...
    await activePoll
    if (forceVerify && !activePollWasForced && !outcome.value) {
...
async function settleExpiredAfterVerification() {
  ...
  try {
    ...
    // The order still transitions to its local expiry state if the final lookup is unavailable.
  }
  if (!outcome.value) {
    cleanup()
    setOutcome('expired')
...
function startCountdown(seconds: number) {
  ...
  if (remainingSeconds.value <= 0) { void settleExpiredAfterVerification(); return }
  countdownTimer = setInterval(() => {
    remainingSeconds.value--
    if (remainingSeconds.value <= 0) { void settleExpiredAfterVerification() }
  }, 1000)
}
...
onMounted(() => {
  if (remainingSeconds.value <= 0 || outcome.value || !isMobileAlipayDeepLink.value) return
