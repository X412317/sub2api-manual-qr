...
describe('decidePaymentLaunch', () => {
  it('routes manual provider orders to the proof submission flow', () => {
    const decision = decidePaymentLaunch(createOrderResult({
      provider_key: 'manual_qr',
      payment_type: 'alipay',
      qr_code: '/api/v1/payment/orders/101/manual-qr',
    ...
    expect(decision.kind).toBe('manual_qr')
    expect(decision.paymentState.providerKey).toBe('manual_qr')
    expect(decision.paymentState.outTradeNo).toBe('sub2_manual_101')
  })
...
describe('readPaymentRecoverySnapshot', () => {
  it('keeps an expired manual order snapshot so proof review can resume', () => {
    const restored = readPaymentRecoverySnapshot(JSON.stringify({
      orderId: 77,
      amount: 50,
      qrCode: '/api/v1/payment/orders/77/manual-qr',
      expiresAt: '2024-01-01T00:10:00.000Z',
      paymentType: 'wxpay',
      payUrl: '',
      outTradeNo: 'sub2_manual_77',
      clientSecret: '',
      intentId: '',
      currency: 'CNY',
      countryCode: '',
      ...
      paymentMode: 'qrcode',
      resumeToken: '',
      providerKey: 'manual_qr',
      createdAt: Date.UTC(2024, 0, 1, 0, 0, 0),
    }), {
      now: Date.UTC(2024, 0, 2, 0, 0, 0),
    })

    expect(restored?.orderId).toBe(77)
    expect(restored?.providerKey).toBe('manual_qr')
  })
