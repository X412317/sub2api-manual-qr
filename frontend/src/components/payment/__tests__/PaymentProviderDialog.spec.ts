...
describe('PaymentProviderDialog payment guide', () => {
  ...
  it('creates manual QR providers disabled with QR-only and refunds off', async () => {
    const wrapper = mountDialog()

    ;(wrapper.vm as unknown as { reset: (key: string) => void }).reset('manual_qr')
    await nextTick()
    await wrapper.find('input[type="text"]').setValue('Manual QR')
    await wrapper.find('form').trigger('submit.prevent')
