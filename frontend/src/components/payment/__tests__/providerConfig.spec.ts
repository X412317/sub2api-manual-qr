...
import {
  ...
  providerSupportsEnabledVisibleMethod,
...
describe('payment provider visible-method mapping', () => {
  it('maps EasyPay instance methods to globally enabled visible methods', () => {
    expect(getProviderVisibleMethods('easypay', ['wxpay'])).toEqual(['wxpay'])
    expect(providerSupportsEnabledVisibleMethod('easypay', ['wxpay'], ['wxpay'])).toBe(true)
    expect(providerSupportsEnabledVisibleMethod('easypay', ['wxpay'], ['alipay'])).toBe(false)
  })

  it('does not treat the easypay provider key as a global payment type', () => {
    expect(providerSupportsEnabledVisibleMethod('easypay', ['alipay'], ['easypay'])).toBe(false)
    expect(providerSupportsEnabledVisibleMethod('manual_qr', ['wxpay'], ['wxpay'])).toBe(true)
  })

  it('normalizes official aliases and Stripe sub-methods', () => {
    expect(getProviderVisibleMethods('alipay', ['alipay_direct'])).toEqual(['alipay'])
    expect(getProviderVisibleMethods('stripe', ['card', 'link'])).toEqual(['stripe'])
  })
})
