...
describe('PaymentStatusPanel', () => {
  ...
  it('actively verifies EasyPay orders even when the visible method is Alipay', async () => {
  ...
  it('performs a final EasyPay verification after expiresAt before showing expiry', async () => {
    vi.setSystemTime(new Date('2026-04-20T13:00:00Z'))
    pollOrderStatus.mockResolvedValue({
      ...orderFactory('PENDING'),
      provider_key: 'easypay',
    ...
    expect(verifyOrder).toHaveBeenCalledWith('sub2_20260420abcd1234')
    expect(wrapper.text()).toContain('payment.result.success')
    expect(wrapper.text()).not.toContain('payment.qr.expiredDesc')
  })

  it('never sends manual QR orders to automatic provider verification', async () => {
    pollOrderStatus.mockResolvedValue({
      ...orderFactory('PENDING'),
      payment_type: 'wxpay',
    ...
    const wrapper = mount(PaymentStatusPanel, {
      props: {
        orderId: 42,
        qrCode: '/api/v1/payment/orders/42/manual-qr',
        expiresAt: '2099-01-01T12:30:00Z',
        paymentType: 'wxpay',
