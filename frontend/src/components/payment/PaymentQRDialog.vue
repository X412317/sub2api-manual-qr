...
async function pollStatus(forceVerify = false): Promise<void> {
  if (!props.orderId || success.value || expired.value) return
  if (pollInFlight) {
    const activePoll = pollInFlight
    const activePollWasForced = pollInFlightUsesForcedVerify
    await activePoll
    if (forceVerify && !activePollWasForced && !success.value && !expired.value) {
  ...
  const activePoll = (async () => {
    ...
    if (!order || success.value || expired.value) return
    order = await tryRecoverPendingOrder(order, forceVerify)
    if (success.value || expired.value) return
...
async function settleExpiredAfterVerification() {
  ...
  try {
    ...
    // The dialog still transitions to its local expiry state if the final lookup is unavailable.
  }
