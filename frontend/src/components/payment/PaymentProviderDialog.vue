  ...
  >
    <form id="provider-form" @submit.prevent="handleSave" class="space-y-4">
      ...
      <div class="flex flex-wrap items-center gap-x-5 gap-y-2">
        ...
        <ToggleSwitch v-if="form.provider_key !== 'manual_qr'" :label="t('admin.settings.payment.refundEnabled')" :checked="form.refund_enabled" @toggle="form.refund_enabled = !form.refund_enabled; if (!form.refund_enabled) form.allow_user_refund = false" />
        <ToggleSwitch v-if="form.provider_key !== 'manual_qr' && form.refund_enabled" :label="t('admin.settings.payment.allowUserRefund')" :checked="form.allow_user_refund" @toggle="form.allow_user_refund = !form.allow_user_refund" />
      ...
      <ManualQRAssetManager
        v-if="form.provider_key === 'manual_qr' && editing"
        :provider-id="editing.id"
      />
      <div
        v-else-if="form.provider_key === 'manual_qr'"
        class="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800 dark:border-amber-800/60 dark:bg-amber-900/20 dark:text-amber-200"
      >
        {{ t('admin.settings.payment.manualQRSaveFirst') }}
      </div>
...
import ManualQRAssetManager from './ManualQRAssetManager.vue'
...
function defaultPaymentMode(providerKey: string): string {
  if (providerKey === 'easypay' || providerKey === 'manual_qr') return PAYMENT_MODE_QRCODE
  return ''
}
...
function isValidPaymentMode(providerKey: string, mode: string): boolean {
  if (providerKey === 'manual_qr') return mode === PAYMENT_MODE_QRCODE
...
const limitableTypes = computed(() => {
  if (form.provider_key === 'manual_qr') return []
...
function onKeyChange() {
  ...
  if (form.provider_key === 'manual_qr') {
    form.enabled = false
...
function handleSave() {
  ...
  emit('save', {
    ...
    payment_mode: form.provider_key === 'manual_qr' ? PAYMENT_MODE_QRCODE : (supportsPaymentMode.value ? form.payment_mode : ''),
    refund_enabled: form.provider_key === 'manual_qr' ? false : form.refund_enabled,
    allow_user_refund: form.provider_key === 'manual_qr' ? false : (form.refund_enabled ? form.allow_user_refund : false),
...
function reset(defaultKey: string) {
  ...
  form.enabled = defaultKey !== 'manual_qr'
...
function loadProvider(provider: ProviderInstance) {
  ...
  if (provider.provider_key === 'manual_qr') {
