...
export function decidePaymentLaunch(
  ...
  if (baseState.providerKey === 'manual_qr') {
    return { kind: 'manual_qr', paymentState: baseState, recovery: baseState }
  }
...
export function readPaymentRecoverySnapshot(
  ...
  try {
    ...
    if (parsed.providerKey !== 'manual_qr' && Number.isFinite(expiresAt) && expiresAt <= now) {
      return null
    }
    ...
    return {
      ...
      providerKey: typeof parsed.providerKey === 'string' ? parsed.providerKey : '',
