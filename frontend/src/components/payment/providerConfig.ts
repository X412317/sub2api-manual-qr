...
export const PROVIDER_SUPPORTED_TYPES: Record<string, string[]> = {
  ...
  airwallex: ['airwallex'],
  manual_qr: ['alipay', 'wxpay'],
}

/**
 * Convert an instance type to the user-facing method key used by global
 * payment settings. Provider keys and visible method keys are different for
 * aggregators such as EasyPay.
 */
export function normalizeProviderVisibleMethod(type: string): string {
  const value = String(type || '').trim().toLowerCase()
  if (value === 'alipay' || value.startsWith('alipay_')) return 'alipay'
  if (value === 'wxpay' || value.startsWith('wxpay_')) return 'wxpay'
  if (value === 'card' || value === 'link') return 'stripe'
  return value
}

/** Return the visible methods claimed by one provider instance. */
export function getProviderVisibleMethods(
  providerKey: string,
  supportedTypes?: string[] | null,
): string[] {
  const key = String(providerKey || '').trim().toLowerCase()
  const configured = Array.isArray(supportedTypes)
    ? supportedTypes.map(normalizeProviderVisibleMethod).filter(Boolean)
    : []
  const configuredSet = new Set(configured)

  switch (key) {
    case 'alipay':
      return configured.length === 0 || configuredSet.has('alipay') ? ['alipay'] : []
    case 'wxpay':
      return configured.length === 0 || configuredSet.has('wxpay') ? ['wxpay'] : []
    case 'stripe':
      return configured.length === 0 || configuredSet.has('stripe') ? ['stripe'] : []
    case 'airwallex':
      return ['airwallex']
    case 'easypay':
      if (supportedTypes == null) return ['alipay', 'wxpay']
      return Array.from(new Set(configured))
    case 'manual_qr':
      if (supportedTypes == null) return ['alipay', 'wxpay']
      return Array.from(new Set(configured))
    default:
      return configured.length > 0 ? Array.from(new Set(configured)) : [key].filter(Boolean)
  }
}

/** Whether at least one method claimed by an instance is globally enabled. */
export function providerSupportsEnabledVisibleMethod(
  providerKey: string,
  supportedTypes: string[] | null | undefined,
  enabledMethods: string[],
): boolean {
  const enabled = new Set(enabledMethods.map(normalizeProviderVisibleMethod))
  return getProviderVisibleMethods(providerKey, supportedTypes)
    .some(method => enabled.has(method))
}
