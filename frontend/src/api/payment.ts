...
export const paymentAPI = {
  ...
  getManualProof(id: number) {
    return apiClient.get<ManualPaymentProof>(`/payment/orders/${id}/manual-proof`)
  },

  submitManualProof(id: number, transactionNo: string, proof: File) {
    const form = new FormData()
    form.append('transaction_no', transactionNo)
    form.append('proof', proof)
    return apiClient.post<ManualPaymentProof>(`/payment/orders/${id}/manual-proof`, form, {
      headers: { 'Content-Type': 'multipart/form-data' }
    })
  },

  getManualQR(id: number) {
    return apiClient.get<Blob>(`/payment/orders/${id}/manual-qr`, { responseType: 'blob' })
  }
}
