...
export const adminPaymentAPI = {
  ...
  getManualReviewSummary() {
    return apiClient.get<{ pending: number }>('/admin/payment/manual-reviews/summary')
  },

  getManualProof(id: number) {
    return apiClient.get<ManualPaymentProof>(`/admin/payment/orders/${id}/manual-proof`)
  },

  getManualProofImage(id: number) {
    return apiClient.get<Blob>(`/admin/payment/orders/${id}/manual-proof`, {
      params: { image: 1 },
      responseType: 'blob'
    })
  },

  reviewManualProof(id: number, data: { action: 'approve' | 'reject'; received_amount?: string; reason?: string }) {
    return apiClient.post<ManualPaymentProof>(`/admin/payment/orders/${id}/manual-review`, data)
  },
  ...
  getManualQRAssets(providerId: number) {
    return apiClient.get<ManualQRAsset[]>(`/admin/payment/providers/${providerId}/manual-qr-assets`)
  },

  saveManualQRAsset(providerId: number, channel: 'alipay' | 'wxpay', amount: number | null, qr: File) {
    const form = new FormData()
    form.append('channel', channel)
    if (amount !== null) form.append('amount', String(amount))
    form.append('qr', qr)
    return apiClient.post<ManualQRAsset>(`/admin/payment/providers/${providerId}/manual-qr-assets`, form, {
      headers: { 'Content-Type': 'multipart/form-data' }
    })
  },

  deleteManualQRAsset(providerId: number, assetId: number) {
    return apiClient.delete(`/admin/payment/providers/${providerId}/manual-qr-assets/${assetId}`)
  },

  getManualQRAssetImage(providerId: number, assetId: number) {
    return apiClient.get<Blob>(`/admin/payment/providers/${providerId}/manual-qr-assets/${assetId}/image`, {
      responseType: 'blob'
    })
  }
}
